...
def parse_args():
    ...
    parser.add_argument(
        "--instance_files",
        nargs="+",
        default=["data/model_generations/machine_generated_instances.jsonl"],
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["data/batch_221203/is_clf_or_not.jsonl"],
    )
    parser.add_argument(
        "--output_dir",
...
def parse_instances_for_classification_task(raw_text, instruction):
    instances = []
    if not "Class label:" in raw_text:
        return []
    instance_texts = raw_text.split("Class label:")[1:] #ignore the part before "Class label:"
    for instance_text in instance_texts:
...
if __name__ == "__main__":
    ...
    if args.num_instructions is not None: #print only num_instructions instances
    ...
    if args.include_seed_tasks: #add seed tasks in traning instances
